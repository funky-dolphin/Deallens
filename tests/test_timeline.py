...
# ---------------------------------------------------------------------------
# Workstream 5 reads these dates rather than deriving its own
# ---------------------------------------------------------------------------

def test_hedging_delay_scenarios_use_the_timeline_dates():
    """
    The assignment requires delay-to-first and delay-to-final-extension
    scenarios. Both take their dates from the timeline, so the close date a
    hedge is priced against is the same one the timeline shows its working for.
    """
    from deallens.analytics.hedging import run_scenarios

    rows = _base_rows(
        extension_conditions="extended for two successive periods of ninety (90) days"
    )
    horizon = build_timeline(rows).horizon
    scenarios = run_scenarios(horizon=horizon)

    delays = [s for s in scenarios if "delayed to" in s["scenario"]]
    assert {s["scenario"] for s in delays} == {
        "Closing delayed to first extension date",
        "Closing delayed to final extension date",
    }
    first = next(s for s in delays if "first" in s["scenario"])
    final = next(s for s in delays if "final" in s["scenario"])
    assert (first["delay_days"], first["delayed_to"]) == (90, "2027-09-23")
    assert (final["delay_days"], final["delayed_to"]) == (180, "2027-12-22")


def test_a_delay_costs_the_forward_starting_hedge_its_carry():
    """The stub reported zero for every strategy, which made the two required
    delay scenarios indistinguishable from the base case."""
    from deallens.analytics.hedging import run_scenarios

    rows = _base_rows(
        extension_conditions="extended for two successive periods of ninety (90) days"
    )
    scenarios = run_scenarios(horizon=build_timeline(rows).horizon)
    hedged = [
        s for s in scenarios
        if s["strategy"] == "Forward-Starting IRS Hedge" and "delayed to" in s["scenario"]
    ]

    assert all(s["net_pnl"] < 0 for s in hedged), "carrying a hedge longer costs money"
    # 180 days of carry costs more than 90.
    assert min(s["net_pnl"] for s in hedged) < max(s["net_pnl"] for s in hedged)


def test_no_timeline_means_no_assumed_delay():
    """An invented delay would put a fabricated timing cost in front of
    someone sizing a hedge."""
    from deallens.analytics.hedging import run_scenarios

    scenarios = run_scenarios(horizon=None)
    delays = [s for s in scenarios if "delayed" in s["scenario"]]

    assert delays, "the scenario is still reported"
    assert all(s["net_pnl"] is None for s in delays)
    assert all(s["delay_days"] is None for s in delays)
    assert all("no delay assumed" in s["note"].lower() for s in delays)


def test_the_carry_cost_uses_the_swap_spread():
    """Previously treasury_rate and swap_rate were read and never used."""
    from deallens.analytics.hedging import compute_carry_cost

    # $4bn, 15bp swap spread, 180 days.
    cost = compute_carry_cost(4_000_000_000, 0.0440, 0.0425, 180)
    assert round(cost) == round(4_000_000_000 * 0.0015 * 180 / 365)
